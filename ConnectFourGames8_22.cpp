...
void dropADisc(char player, char board[][7])
{
	...
	do
	{
		cout << "Drop a " << (player == 'R' ? "red" : "yellow") << "disk at column(0-6): ";
		int column;
		cin >> column;
		...
		if (placeADisk(board, column, player))
		{
			...
			cout << "This column is full. Tray a different column" << endl;
	} 
	while (!done);
}
...
bool isConsecutiveFour(char values[][7])
{
	...
	for (int i = 0; i < numberOfRows - 3; i++)
	{
		...
		for (int k = 0; k < maxNumberOfElementsInDiagonal; k++)
			diagonal[k] = values[k + i][k];

		if (isConsecutiveFour(diagonal, maxNumberOfElementsInDiagonal))
			return true;
	}
